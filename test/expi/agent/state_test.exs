defmodule Expi.Agent.StateTest do
  ...
  defp mock_tool do
    {:ok, tool} = Expi.Agent.Tool.new(
      ...
      "Test Tool",
      fn _, _, _, _ -> {:ok, %Expi.Agent.Types.AgentToolResult{content: [], details: %{result: "test result"}}} end
    )
    tool
  end
  ...
  describe "add_tool/2" do
    ...
    test "preserves existing tools" do
      {:ok, tool1} = Expi.Agent.Tool.new("tool1", "First", %{type: :object}, "Tool 1", fn _, _, _, _ -> {:ok, %Expi.Agent.Types.AgentToolResult{content: [], details: %{}}} end)
      {:ok, state} = State.new(mock_model(), %{tools: [tool1]})
      
      {:ok, tool2} = Expi.Agent.Tool.new("tool2", "Second", %{type: :object}, "Tool 2", fn _, _, _, _ -> {:ok, %Expi.Agent.Types.AgentToolResult{content: [], details: %{}}} end)
  ...
  describe "remove_tool/2" do
    ...
    test "preserves other tools when removing one" do
      {:ok, tool1} = Expi.Agent.Tool.new("keep", "Keep me", %{type: :object}, "Keep Tool", fn _, _, _, _ -> {:ok, %Expi.Agent.Types.AgentToolResult{content: [], details: %{}}} end)
      {:ok, tool2} = Expi.Agent.Tool.new("remove", "Remove me", %{type: :object}, "Remove Tool", fn _, _, _, _ -> {:ok, %Expi.Agent.Types.AgentToolResult{content: [], details: %{}}} end)
  ...
  describe "edge cases and error handling" do
    test "handles large message histories" do
      ...
      # Add many messages
      large_state = Enum.reduce(1..1000, state, fn i, acc ->
