defmodule Expi.Agent.ToolTest do
    ...
    {:ok, tool} = Expi.Agent.Tool.new(
      ...
      fn _tool_call_id, params, _abort_signal, _update_callback ->
        ...
        if String.length(query) > 0 do
          {:ok, %Expi.Agent.Types.AgentToolResult{
            content: [%Expi.Types.TextContent{type: :text, text: "Search results for: #{query}"}],
            details: %{query: query}
          }}
        else
  ...
  describe "call/2" do
    ...
    test "handles invalid argument types" do
      tool = mock_search_tool()
      args = %{"query" => 123}  # Should be string, not integer
  ...
  describe "validate/1" do
    ...
    test "detects invalid function arity" do
      tool = %Tool{
        name: "valid_name",
        description: "Valid description",
        function: fn -> {:ok, "no args"} end  # Wrong arity
      }
  ...
  describe "complex tool scenarios" do
    test "tool with complex argument processing" do
      ...
      # Test sum operation
      assert {:ok, result} = Tool.call(complex_tool, %{
        "data" => [1, 2, 3, 4, 5],
        "operation" => "sum"
      })
      ...
      assert {:error, error} = Tool.call(complex_tool, %{
        "data" => [1, 2, 3],
        "operation" => "unknown"
      })
  ...
  describe "error handling and edge cases" do
    ...
    test "handles tool with invalid return format" do
      invalid_tool = %Tool{
        ...
        function: fn _args ->
          "not a tuple"  # Should return {:ok, result} or {:error, reason}
        end
      }
