defmodule Expi.Agent.ToolExecutorTest do
  ...
  describe "execute_tools_concurrent/4" do
    test "executes multiple tools concurrently" do
      ...
      # Should be fast for simple operations
    ...
    test "handles mixed success and failure in concurrent execution" do
      tool_calls = [
        ...
        # Failure
        mock_tool_call("calculator", %{"expression" => "invalid"}),
        # Success
    ...
    test "respects concurrency limit" do
      ...
      assert {:ok, results} =
               ToolExecutor.execute_tools_concurrent(
                 tool_calls,
                 tools,
                 max_concurrent: 3,
                 timeout: 5000
    ...
    test "handles timeout in concurrent execution" do
      ...
      tool_calls = [
        # Fast
        mock_tool_call("slow_tool", %{"delay" => 50}),
        # Medium
        mock_tool_call("slow_tool", %{"delay" => 100}),
        # Slow (will timeout)
        mock_tool_call("slow_tool", %{"delay" => 2000})
      ]
  ...
  describe "execute_tools_sequential/3" do
    ...
    test "stops on first error when configured" do
      ...
      {:ok, results} =
        ToolExecutor.execute_tools_sequential(
          tool_calls,
          tools,
          stop_on_error: true
        )
    ...
    test "continues after errors when configured" do
      tool_calls = [
        ...
        mock_tool_call("calculator", %{"expression" => "invalid"}),
        # Will succeed
        mock_tool_call("search", %{"query" => "should execute"})
      ]
  ...
  describe "error isolation" do
    ...
    test "prevents one failing tool from affecting others" do
      ...
      tool_calls = [
        # Should work
        mock_tool_call("stateful", %{"action" => "increment"}),
        # Should fail
        mock_tool_call("stateful", %{"action" => "crash"}),
        # Should work
        mock_tool_call("stateful", %{"action" => "read"})
      ]
  ...
  describe "edge cases" do
    test "handles tool with complex argument validation" do
      ...
      test_cases = [
        # Should succeed
        {%{"data" => [1, 2, 3], "rules" => %{"min" => 1}}, false},
        # Should fail - missing rules
        {%{"data" => [1, 2, 3]}, true},
        # Should fail - missing data
        {%{"rules" => %{"min" => 1}}, true},
        # Should fail - missing both
        {%{}, true}
      ]
