defmodule Expi.Agent.EventsTest do
  use ExUnit.Case, async: false  # Events may involve process communication
  ...
  describe "tool_execution_event/4" do
    ...
    test "creates tool update event with partial results" do
      event = Events.tool_execution_event(
        :update, 
        "call_456", 
        "search", 
        %{"query" => "test"}, 
  ...
  describe "event filtering and batching" do
    test "filters events by type" do
      ...
      Events.emit_event(Events.agent_lifecycle_event(:start, mock_agent_state()), [filtered_callback])
      Events.emit_event(Events.turn_lifecycle_event(:start, 1), [filtered_callback])
      Events.emit_event(Events.message_lifecycle_event(:start, Message.user("Test")), [filtered_callback])
  ...
  describe "complex event scenarios" do
    test "handles rapid event emission" do
      ...
      for i <- 1..100 do
        event = Events.message_lifecycle_event(:update, 
      ...
      assert count >= 90  # Allow for some timing variations
    end
  ...
  describe "event validation and structure" do
    ...
    test "event timestamps are monotonically increasing" do
      events = for i <- 1..10 do
        Process.sleep(1)  # Ensure time progression
