defmodule Expi.Agent.SteeringTest do
  ...
  describe "should_process_steering/3" do
    ...
    test "defers steering when agent is streaming" do
      ...
      decision =
        Steering.should_process_steering(agent_state, steering_messages, interrupt_tools: false)
    ...
    test "processes urgent steering even when busy" do
      ...
      decision =
        Steering.should_process_steering(agent_state, urgent_messages, interrupt_tools: true)
    ...
    test "defers when too many steering messages" do
      ...
      decision =
        Steering.should_process_steering(agent_state, many_messages, max_steering_per_turn: 5)
    ...
    test "respects interrupt_tools option" do
      ...
      with_interrupt =
        Steering.should_process_steering(agent_state, steering_messages, interrupt_tools: true)
  ...
  describe "should_process_follow_up/3" do
    test "processes follow-up when agent is idle and enough time passed" do
      ...
      decision =
        Steering.should_process_follow_up(agent_state, follow_up_messages, min_idle_time_ms: 100)
    ...
    test "defers when too many follow-up messages" do
      ...
      decision =
        Steering.should_process_follow_up(agent_state, many_messages, max_follow_ups_per_turn: 3)
    ...
    test "respects minimum idle time" do
      ...
      decision =
        Steering.should_process_follow_up(agent_state, follow_up_messages,
          # 10 seconds
    ...
    test "detects natural conversation breaks" do
      ...
      decision =
        Steering.should_process_follow_up(idle_agent, follow_up_messages,
          natural_break_detection: true,
          # No time requirement for this test
          min_idle_time_ms: 0
        )
  ...
  describe "apply_steering_logic/3" do
    ...
    test "queues steering for after tools when appropriate" do
      ...
      {decision, updated_queue} =
        Steering.apply_steering_logic(busy_agent, message_queue, interrupt_tools: false)
    ...
    test "defers steering when timing is not optimal" do
      ...
      queue_with_many =
        Enum.reduce(many_messages, message_queue, fn msg, acc ->
          Queue.add_steering(acc, msg)
        end)

      {decision, _updated_queue} =
        Steering.apply_steering_logic(agent_state, queue_with_many, max_steering_per_turn: 3)
  ...
  describe "apply_follow_up_logic/3" do
    test "processes follow-up at natural breaks" do
      ...
      {decision, updated_queue} =
        Steering.apply_follow_up_logic(idle_agent, message_queue, min_idle_time_ms: 0)
    ...
    test "defers when not at natural conversation break" do
      ...
      {decision, _updated_queue} =
        Steering.apply_follow_up_logic(agent_state, message_queue,
  ...
  describe "update_processing_state/3" do
    test "updates state after interruption coordination" do
      ...
      updated_state =
        Steering.update_processing_state(
          agent_state,
          coordination_result,
          []
        )
  ...
  describe "edge cases and error handling" do
    test "handles malformed steering messages gracefully" do
      ...
      edge_messages = [
        ...
        Message.user("   "),
        # Very long content
        Message.user(String.duplicate("A", 10_000))
      ]
    ...
    test "handles complex queue states" do
      ...
      complex_queue =
        Enum.reduce(1..50, complex_queue, fn i, acc ->
          if rem(i, 2) == 0 do
