defmodule ExpiAi.AI.BenchmarkTest do
      ...
      if Enum.empty?(providers) do
        ...
        for {provider, model_id} <- providers do
          ...
          latencies = for _i <- 1..5 do
            {time_us, {:ok, _response}} = :timer.tc(fn ->
              AI.complete_simple(model, context)
            end)
          ...
          results = Map.put(results, "#{provider}/#{model_id}", %{
            avg: avg_latency,
            min: min_latency,
            max: max_latency,
            samples: latencies
    ...
    test "concurrent request throughput" do
      ...
      if is_nil(provider) do
        ...
        for concurrency <- concurrency_levels do
          ...
          throughput = length(responses) / (duration_ms / 1000)  # requests per second
          
          results = Map.put(results, concurrency, %{
        ...
        assert optimal_stats.throughput > 0.1, "Throughput too low: #{optimal_stats.throughput} req/s"
      end
    end
  end
  ...
  describe "Streaming Benchmarks" do
    ...
    test "streaming vs synchronous performance" do
      ...
      if is_nil(provider) do
        ...
        {sync_time_us, {:ok, sync_response}} = :timer.tc(fn ->
          AI.complete_simple(model, context)
        end)
        ...
        {stream_time_us, stream_content_length} = :timer.tc(fn ->
        ...
        IO.puts("   Sync - Time: #{Float.round(sync_time_ms, 1)}ms, Length: #{sync_content_length} chars")
        IO.puts("   Stream - Time: #{Float.round(stream_time_ms, 1)}ms, Length: #{stream_content_length} chars")
        ...
        if sync_content_length > 0 and stream_content_length > 0 do
          ...
          assert efficiency_ratio > 0.5, "Streaming too slow compared to sync: #{efficiency_ratio}"
        end
      end
    end
    ...
    test "streaming event processing performance" do
      ...
      assert events_per_second > 1000, "Event processing too slow: #{events_per_second} events/sec"
    end
  end
  ...
  describe "Connection Pool Benchmarks" do
    test "connection pool efficiency" do
      ...
      if is_nil(provider) do
        ...
        {sequential_time_us, _} = :timer.tc(fn ->
          for _i <- 1..5 do
            AI.complete_simple(model, context)
          end
        end)
        ...
        {concurrent_time_us, _} = :timer.tc(fn ->
          tasks = for _i <- 1..5 do
    ...
    test "cost per token across providers" do
      ...
      if Enum.empty?(providers) do
        ...
        for {provider, model_id} <- providers do
          ...
          cost_per_token = if total_tokens > 0 do
          ...
          cost_per_char = if content_length > 0 do
  ...
  defp get_available_providers do
    ...
    # Check Anthropic
    providers = if System.get_env("ANTHROPIC_API_KEY") do
    ...
    # Check Google
    providers = if System.get_env("GOOGLE_API_KEY") do
    ...
    # Check Ollama
    providers = if test_ollama_connection() == :ok do
