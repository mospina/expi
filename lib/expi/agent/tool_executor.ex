defmodule Expi.Agent.ToolExecutor do
  ...
  def execute_tools_concurrent(tool_calls, available_tools, options \\ []) do
    ...
    case strategy do
      :concurrent -> 
        execute_concurrent(tool_calls, available_tools, execution_id, timeout, max_concurrent, on_update, on_complete)
      :sequential -> 
        execute_sequential(tool_calls, available_tools, execution_id, timeout, on_update, on_complete)
      :hybrid -> 
        execute_hybrid(tool_calls, available_tools, execution_id, timeout, max_concurrent, on_update, on_complete)
    end
  end
  ...
  def execute_single_tool(tool_call, tool, options \\ []) do
    ...
    update_callback = if on_update do
      fn partial_result ->
    ...
    try do
      ...
      case Expi.Agent.Tool.execute(
        ...
        {:error, reason} ->
          ...
          {:ok, error_result}  # Don't propagate errors - wrap in result
      end
    rescue
      error ->
  ...
  def aggregate_tool_results(results, metadata \\ %{}) do
    ...
    %{
      ...
      average_execution_time: if length(execution_times) > 0 do
  ...
  @spec execute_concurrent([ToolCall.t()], [AgentTool.t()], execution_id(), pos_integer(), 
                          pos_integer(), AgentToolCallback.t() | nil, function() | nil) :: 
        execution_result()
  defp execute_concurrent(tool_calls, available_tools, execution_id, timeout, max_concurrent, on_update, on_complete) do
    with :ok <- validate_execution_setup(tool_calls, available_tools) do
      ...
      |> Task.async_stream(
        ...
        timeout: timeout + 1000,  # Slightly longer than individual tool timeout
  ...
  @spec execute_sequential([ToolCall.t()], [AgentTool.t()], execution_id(), pos_integer(),
                          AgentToolCallback.t() | nil, function() | nil) :: execution_result()
  defp execute_sequential(tool_calls, available_tools, execution_id, timeout, on_update, on_complete) do
    with :ok <- validate_execution_setup(tool_calls, available_tools) do
      tool_map = available_tools
      |> Enum.map(fn tool -> {AgentTool.name(tool), tool} end)
      |> Map.new()
  ...
  @spec execute_hybrid([ToolCall.t()], [AgentTool.t()], execution_id(), pos_integer(),
                      pos_integer(), AgentToolCallback.t() | nil, function() | nil) :: 
        execution_result()
  defp execute_hybrid(tool_calls, available_tools, execution_id, timeout, max_concurrent, on_update, on_complete) do
