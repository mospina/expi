defmodule Expi.Agent.Events do
  ...
  """
  @spec emit_event(AgentEvent.t(), [Callbacks.callback_spec()] | event_emitter(), emission_options()) :: 
        :ok | {:error, any()}
        ...
        tasks = Enum.map(events, fn event ->
          Task.async(fn ->
            emit_event(event, callbacks_or_emitter, 
              ...
              async: false  # Individual emissions are sync within the task
            )
          end)
        end)
  ...
  def broadcast_event(event, emitters, options \\ []) do
    ...
    emission_options = case priority do
  ...
  def detach_emitter(%{callbacks: callbacks} = emitter, callback_or_id) do
    updated_callbacks = case callback_or_id do
  ...
  """
  @spec route_events([AgentEvent.t()], [{event_filter(), [Callbacks.callback_spec()]}], keyword()) :: :ok
  ...
  """
  @spec agent_lifecycle_event(:start | :end | :error, AgentState.t(), String.t() | nil) :: AgentEvent.t()
  def agent_lifecycle_event(lifecycle_type, agent_state, _error_message \\ nil) do
    case lifecycle_type do
      ...
      :end -> AgentEvent.agent_end(agent_state.messages)
      :error -> 
  ...
  defp validate_event_fields(event) do
    case event.type do
      ...
      _ ->
        :ok  # Basic validation for other event types
    end
  end
