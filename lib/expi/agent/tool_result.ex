defmodule Expi.Agent.ToolResult do
  ...
  def merge_results(results1, results2, options \\ []) do
    ...
    resolved =
      Enum.map(grouped, fn {_id, duplicate_results} ->
  ...
  @spec estimate_content_block_size(TextContent.t() | ImageContent.t() | map()) ::
          non_neg_integer()
  ...
  # Default estimate
  defp estimate_content_block_size(_), do: 100
    ...
    if length(timeout_results) > length(results) * 0.3 do
      ...
      [
        %{
          ...
          description:
            "Frequent timeouts detected in #{length(timeout_results)} of #{length(results)} executions",
  ...
  defp format_for_console(results) do
    ...
    header =
      "Tool Execution Results (#{stats.total_count} total, #{stats.success_count} successful)"
    ...
    summary =
      "Success Rate: #{trunc(stats.success_rate * 100)}% | " <>
  ...
  defp format_for_log(results) do
    Enum.map(results, fn result ->
      %{
        ...
        message:
          "Tool #{result.tool_name} #{if result.is_error, do: "failed", else: "completed"}",
        metadata: %{
          tool_call_id: result.tool_call_id,
          tool_name: result.tool_name,
          execution_time: get_execution_time(result),
  ...
  defp check_required_fields(results) do
    ...
    |> Enum.flat_map(fn {result, index} ->
      ...
      result_issues =
        if is_nil(result.tool_call_id) or result.tool_call_id == "" do
      ...
      result_issues =
        if is_nil(result.tool_name) or result.tool_name == "" do
      ...
      result_issues =
        if is_nil(result.content) or result.content == [] do
  ...
  defp generate_recommendations(analysis) do
    ...
    recommendations =
      if analysis.statistics.success_rate < 0.8 do
    ...
    recommendations =
      if analysis.statistics.average_execution_time > 10_000 do
