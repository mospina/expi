defmodule Expi.Agent.Context do
  ...
  def compose_transforms(transform_functions) when is_list(transform_functions) do
    fn messages, abort_signal ->
      Enum.reduce_while(transform_functions, {:ok, messages}, fn transform_fn,
                                                                 {:ok, current_messages} ->
  ...
  def analyze_context(messages) when is_list(messages) do
    type_distribution =
      Message.group_by_type(messages)
  ...
  defp maybe_prune_for_model(messages, model_type, max_tokens) do
    ...
    if current_tokens > max_tokens do
      ...
      preserve_count =
        case model_type do
          # Claude handles longer contexts well
          :claude -> 8
          # Smaller models need aggressive pruning
          :llama_8b -> 3
          # Default
          _ -> 5
        end

      MessageProcessor.prune_by_tokens(messages, max_tokens, preserve_count: preserve_count)
    else
      {:ok, messages}
    end
  end
