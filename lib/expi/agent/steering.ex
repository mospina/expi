defmodule Expi.Agent.Steering do
  ...
  **Characteristics:**
  ...
  @spec apply_follow_up_logic(AgentState.t(), Queue.message_queue(), follow_up_options()) ::
        {{:process_natural, [Message.t()]} | {:wait_for_break, pos_integer()} | 
  ...
  @spec get_processing_priority([Message.t()], [Message.t()]) :: %{
    ...
    recommendation: :process_steering_first | :process_follow_up_first | 
                   :process_together | :defer_all,
    reasoning: String.t()
  }
  def get_processing_priority(steering_messages, follow_up_messages) do
    ...
    {recommendation, reasoning} = cond do
  ...
  def coordinate_with_loop(agent_state, message_queue, options \\ []) do
    ...
    {steering_decision, steering_queue} = apply_steering_logic(agent_state, message_queue, steering_opts)
    {follow_up_decision, final_queue} = apply_follow_up_logic(agent_state, steering_queue, follow_up_opts)
    
    # Determine coordinated action
    {action, messages, mode, estimated_time} = case {steering_decision, follow_up_decision} do
      {{:interrupt, steering_msgs}, _} ->
        # Steering interruption takes precedence
        {:interrupt_loop, steering_msgs, :one_at_a_time, estimate_processing_time(steering_msgs)}
      ...
      {_, {:process_natural, follow_up_msgs}} ->
        # Process follow-ups at natural break
        {:queue_for_next_turn, follow_up_msgs, :one_at_a_time, estimate_processing_time(follow_up_msgs)}
  ...
  def update_processing_state(agent_state, coordination_result, events \\ []) do
    ...
    updated_state = if messages_to_add != [] do
    ...
    final_state = case coordination_result.action do
      :interrupt_loop ->
  ...
  defp build_processing_context(agent_state) do
    ...
    idle_time = if last_message_time do
  ...
  defp has_expired_follow_ups?(messages) do
    expiry_time = System.system_time(:millisecond) - 600_000  # 10 minutes
  ...
  defp determine_steering_processing_mode(messages, options) do
    ...
    if length(messages) <= max_per_batch and has_urgent_steering?(messages) do
      :all  # Process all urgent messages at once
    else
      :one_at_a_time  # Process gradually for non-urgent
    end
  end
  ...
  defp calculate_message_priority(messages, message_type) do
    base_priority = case message_type do
      :steering -> 100
      :follow_up -> 50
    end
  ...
  defp estimate_completion_time(agent_state) do
    ...
    # Rough estimates
    tool_time = pending_tools * 2000  # 2 seconds per tool
    streaming_time = if streaming, do: 5000, else: 0  # 5 seconds for streaming
